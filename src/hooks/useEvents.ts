...
const MUTATION_BUSY_MESSAGE = '事件操作正在进行中';
...
export function useEvents() {
  ...
  useEffect(() => {
    ...
    setError(null);
    setLoading(true);
    void loadData(targetGeneration, targetMutationVersion, '事件列表加载失败').catch(() => undefined);
    return () => {
  ...
  const load = useCallback(async () => {
    ...
    await loadData(targetGeneration, targetMutationVersion, '事件列表加载失败');
  }, [loadData]);
  ...
  const mutate = useCallback(async (
    ...
    try {
      ...
      if (isCurrent(mounted, generation, targetGeneration)) {
        ...
        } catch { /* write succeeded, refresh error exposed separately */ }
      }
    } finally {
